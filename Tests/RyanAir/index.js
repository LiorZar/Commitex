...
function Fare(td, f, t) {
  const diffTime = Math.abs(t.day - f.day);
  const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
  const fare = f.price + t.price;
  td.appendChild(document.createTextNode(`${diffDays}, ${fare}E`));
  if (fare > 150) td.style.background = "red";
